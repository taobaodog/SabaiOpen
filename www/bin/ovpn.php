...
$password=$_REQUEST['VPNpassword'];
 if(array_key_exists('conf',$_REQUEST)){
  ...
  exec("sed -ir 's\auth-user-pass.*$\auth-user-pass /www/usr/auth-pass\g' /www/usr/ovpn.current");
...
switch ($act){
	case "start":
		if(!file_exists("/www/usr/ovpn.current")){ echo "res={ sabai: false, msg: 'OpenVPN file missing.' };"; break; }
	case "stop":
	...
	case "log": echo (file_exists("/www/stat/ovpn.log") ? str_replace(array("\"","\r"),array("'","\n"),file_get_contents("/www/stat/ovpn.log")) : "No log."); break;
}
