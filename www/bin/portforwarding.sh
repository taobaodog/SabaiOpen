...
#cleanup old PF rules
while [ $i -le $(($num_items-1)) ]
do
	echo "checking rule  #$i:"
	current_rule="`uci get firewall.@redirect[0].name`"
	echo "current rule = $current_rule"
	if [ "$current_rule" = "pforwarding" ]; then
		echo "uci delete firewall.@redirect[0]"
...
do
        ...
        if [ $pfenable = "on" ]; then
                ...
                uci set firewall.@redirect[${i}].name="pforwarding"
			...
			ppptp=$(pgrep pppd)
			ovpn=$(pgrep openvpn)
			if [[ $pptp != 0 ]] && [[ $ovpn != 0 ]]; then
				echo "No VPN service detected"
				echo "condition 3" >> /tmp/portforwarding
				break
			fi
                        uci set firewall.@redirect[${i}].src="wan"
                        uci set firewall.@redirect[${i}].dest="lan"
