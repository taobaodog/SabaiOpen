         ...
         for( var i = 1; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          for(var j = 0; j < columnDefs.length; j++){
          	...
          	if(columnDefs[j].type.includes("text")){
          		data += "<input type='" + columnDefs[j].type + "' required id='" + columnDefs[j].title + "'  pattern='" + columnDefs[j].pattern + "'  title='" + columnDefs[j].msg + "' name='" + columnDefs[j].title + "' placeholder='" + columnDefs[j].title + "' style='overflow:hidden'  class='form-control  form-control-sm' value='" + adata.data()[0][columnDefs[j].name] + "'>";
          	}
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            ...
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button>\
               <input type='submit' form='altEditor-form' data-content='remove' class='btn btn-primary' id='editRowBtn'></input>");
       ...
       _openAddModal: function ( )
       {
         ...
         for( var i = 1; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          for(var j = 0; j < columnDefs.length; j++){
          	...
          	if(columnDefs[j].type.includes("text")){
          		data += "<input type='" + columnDefs[j].type + "' required id='" + columnDefs[j].title + "'  pattern='" + columnDefs[j].pattern + "'  title='" + columnDefs[j].msg + "' name='" + columnDefs[j].title + "' placeholder='" + columnDefs[j].title + "' style='overflow:hidden'  class='form-control  form-control-sm' value=''>";
          	}
          	if(columnDefs[j].type.includes("readonly")){
          		data += "<input type='text' readonly  id='" + columnDefs[j].title + "' name='" + columnDefs[j].title + "' placeholder='" + columnDefs[j].title + "' style='overflow:hidden'  class='form-control  form-control-sm' value=''>";
          	}
          	if(columnDefs[j].type.includes("select")){
          		var options = "";
          		for (var i = 0; i < columnDefs[j].options.length; i++) {
        ...
        //  initValidation();
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            ...
            $('#altEditor-modal').find('.modal-body').html(data);
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button>\
               <input type='submit' form='altEditor-form'  data-content='remove' class='btn btn-primary' id='addRowBtn'></input>");
          });
...
//console.log("JSONData: " +JSON.stringify(comepleteJsonData))
...
//Loading validator file for input validation.
$.getScript("/home/kasper/Work/Test work/libs/jquery-validation-1.15.0/dist/jquery.validate.js", function(data, textStatus){
...
// NTP Server address validator
jQuery.validator.addMethod('serverText', function(value){
...
/*console.log("Data without stringify:")
console.log(data)*/
/*$.ajax({
